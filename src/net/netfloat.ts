...
export function toNetObject(o: Record<string, unknown>): Record<string, unknown> {
    return Object.fromEntries(Object.entries(o).map<[string, unknown]>(([key,v]) => {
        if (typeof v === "number" && !Number.isInteger(v)) {
        return [key, toNetworkFloat(v)];
        } else if (Array.isArray(v)) {
        return [key, v.map(v2 => typeof v2 === "number" && !Number.isInteger(v2) ? toNetworkFloat(v2) : v2)];
...
export function fromNetObject(o: Record<string, unknown>): unknown {
    function isNF(v: unknown): v is NetworkFloat {
      return (v !== null && typeof v === "object" && 'nf' in v && v.nf === true) || false;
    }

    return Object.fromEntries(Object.entries(o).map<[string, unknown|NetworkFloat]>(([key,v]) => {
      if (isNF(v)) {
