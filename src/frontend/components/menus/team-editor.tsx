...
async function scaleFile (file: File, { maxWidth, maxHeight }: {maxWidth: number, maxHeight: number}) {
  const img = new Image();
...
export function TeamEditor({team: initialTeam, onChange}: {team: StoredTeam, onChange: (team: StoredTeam) => void}) {
  ...
  useEffect(() => {
    ...
    return () => { if (tempBlobUrl) { URL.revokeObjectURL(tempBlobUrl) }}
  }, [tempBlobUrl]);

  const onFlagUpload: JSX.GenericEventHandler<HTMLInputElement> = useCallback((evt) => {
    const [imageFile] = (evt.target as HTMLInputElement).files || [];
    if (!imageFile) {
      return;
    }
    scaleFile(imageFile, { maxHeight: 48, maxWidth: 48}).then((blob) => {
  ...
  return <section className={styles.teamEditor}>
    <input className={styles.editable} type="text" value={team.name} onChange={v => {
    ...
    <ol>
      {team.worms.map((wormName, i) => <li key={i}><input minLength={3} maxLength={16} onChange={v => {
        const value = ((v.target as HTMLInputElement).value);
        if (value.length < 3 || value.length > 16) {
          return;
        }
      ...
      }} type="text" value={wormName}></input></li>)}
    </ol>
    <div>
      <button onClick={() => uploadRef.current?.click()}>Upload Flag</button>
      <input ref={uploadRef} hidden onChange={onFlagUpload} type="file" accept="image/jpeg,image/png,image/webp" />
...
export default function TeamEditorMenu({ onGoBack }: Props) {
  const [localTeams, setLocalTeams] = useLocalStorageState<StoredTeam[]>(WORMGINE_STORAGE_KEY_TEAMS, {
  ...
  const onCreateTeam = useCallback(() => {
    ...
    setLocalTeams((t: StoredTeam[]) => [...t, { 
      name: newTeamName,
      worms: Array.from({length: MAX_WORM_NAMES}).map((_, i) => `Worm #${i+1}`),
      synced: false,
    }]);
  ...
  const onTeamSelected = useCallback((evt: JSX.TargetedEvent<HTMLSelectElement>) => {
  ...
  if (localTeams.length) {
    ...
    const onTeamChanged = useCallback((t: StoredTeam) => {
      setLocalTeams((existing: StoredTeam[]) => {existing[selectedTeam] = t; return existing;});
      console.log("Updated team", t);
    }, [selectedTeam]);

    content = <>
      <label for="team-selector">Selected Team </label>
      <select id="team-selector" value={selectedTeam} onChange={onTeamSelected}>
        {localTeams.map((t, i) => <option key={t.name} value={i}>{t.name}</option>)}
      </select>
      <TeamEditor team={localTeams[selectedTeam]} onChange={onTeamChanged} />
      <button disabled={localTeams.length > MAX_TEAMS} onClick={onCreateTeam}>Create Team</button>
