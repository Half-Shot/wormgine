...
export function Lobby({client, gameRoomId: initialGameRoomId, onOpenIngame}: Props) {
    ...
    const lobbyLink = useMemo(() =>
        gameRoomId && `${window.location.origin}${window.location.pathname}?gameRoomId=${encodeURIComponent(gameRoomId)}`
    , [gameRoomId]);
    ...
    if (!gameInstance) {
        if (!gameRoomId) {
            return <main className="menu">
                ...
                <button onClick={createGameCallback} disabled={!!gameRoomId}>Create new Game</button>
            </main>;
        }
        else  {
            return <main className="menu">
    ...
    return <main className="menu">
        ...
        <p>
            You can invite players by sending them a link to <a href={lobbyLink}>{lobbyLink}</a>.
        </p>
        <ul>
            {Object.entries(gameInstance.members).map(([userId, displayname]) => {
                return <li>{displayname} {userId === gameInstance.hostUserId ? <span title="Host">🌟</span> : null}</li>
            })}
        </ul>
        <button onClick={startGame} disabled={!gameInstance.isHost}>Start Game</button>
    </main>;
}